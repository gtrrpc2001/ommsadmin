...
class MonitorProcessSettingScreen extends StatefulWidget {
  const MonitorProcessSettingScreen({
    Key? key,
    required this.idx,
  }) : super(key: key);
  ...
  @override
  _MonitorProcessSettingScreenState createState() =>
      _MonitorProcessSettingScreenState();
}

class _MonitorProcessSettingScreenState
    extends State<MonitorProcessSettingScreen> {
  bool isLoading = true;
  // ignore: unused_field
  ...
  Widget uiField() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          SizedBox(width: 85, child: uiText4("사용", cFont: Colors.black45),),
          SizedBox(width: 155, child: uiText4("설명", cFont: Colors.black45),),
          SizedBox(width: 255, child: uiText4("프로세스명(서비스명 or 실행파일명(exe포함))", cFont: Colors.black45),),
          SizedBox(width: 85, child: uiText4("감지시간(초)", cFont: Colors.black45),),
          SizedBox(width: 85, child: uiText4("반복횟수(회)", cFont: Colors.black45),),
          SizedBox(width: 125, child: uiText4("이상대응방법", cFont: Colors.black45),),
        ],
      ),
    );
  }
            ...
            children: [
              ...
              SizedBox(width: 60,child:
                ElevatedButton(
                  onPressed: () async {
                    ...
                    dic["서비스명"] = "'${getEdit("${sidx}.서비스명")!.text}'";
                    dic["프로세스명"] = "'${getEdit("${sidx}.프로세스명")!.text}'";
                    dic["감지시간"] = "${getEdit("${sidx}.감지시간")!.text}";
                    dic["반복횟수"] = "${getEdit("${sidx}.반복횟수")!.text}";
                    dic["이상대응방법"] = "'${dicKeyValue["${sidx}.이상대응방법"]}'";

                    await ca.dicToTable("모니터링설정_서비스", dic,
                        where: " idx = ${sidx}");
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  String sql = "delete from 모니터링설정_서비스 where idx = ${sidx}";
                  ca.sqlExecPost(sql);
  ...
  Future<bool> doReload() async {
    ...
    try {
      ...
      String sql = "select * from 모니터링설정_서비스 where 정책ID = '${widget.idx}'";
