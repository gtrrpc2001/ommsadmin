...
class MonitorScriptSettingScreen extends StatefulWidget {
  const MonitorScriptSettingScreen({
    Key? key,
    required this.idx,
  }) : super(key: key);
  ...
  @override
  _MonitorScriptSettingScreenState createState() =>
      _MonitorScriptSettingScreenState();
}

class _MonitorScriptSettingScreenState
    extends State<MonitorScriptSettingScreen> {
  bool isLoading = true;
  // ignore: unused_field
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(
            width: 155,
            child: uiText4("설명", cFont: Colors.black45),
          ),
          SizedBox(
            width: 255,
            child: uiText4("스크립트경로", cFont: Colors.black45),
          ),
          SizedBox(
            width: 85,
            child: uiText4("감지시간(초)", cFont: Colors.black45),
          ),
          SizedBox(
            width: 85,
            child: uiText4("반복횟수(회)", cFont: Colors.black45),
          ),
          SizedBox(
            width: 125,
            child: uiText4("이상대응방법", cFont: Colors.black45),
          ),
        ],
      ),
    );
          ...
          Row(
            children: [
              ...
              uiEdt(
                "서비스/스크립트경로",
                "${sidx}.스크립트경로",
                description: "경로 없이 스크립트 코드로 작성 가능",
                width: 250,
              ),
              SizedBox(
                width: 5,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    String spath = getEdit("${sidx}.스크립트경로")!.text;
                    spath = base64.encode(utf8.encode(spath));

                    String scode = getEdit("${sidx}.스크립트코드")!.text;
                    scode = base64.encode(utf8.encode(scode));

                    Map<String, String> dic = Map();
                    dic["사용"] = dicKeyValue["${sidx}.사용"]!;
                    dic["스크립트설명"] = "'${getEdit("${sidx}.스크립트설명")!.text}'";
                    dic["스크립트경로"] = "'${spath}'";
                    dic["스크립트코드"] = "'${scode}'";
                    dic["스크립트감지간격"] = "${getEdit("${sidx}.스크립트감지간격")!.text}";
                    dic["스크립트감지정상"] = "${getEdit("${sidx}.스크립트감지정상")!.text}";
                    dic["스크립트감지오류"] = "${getEdit("${sidx}.스크립트감지오류")!.text}";
                    dic["재시도횟수"] = "${getEdit("${sidx}.재시도횟수")!.text}";
                    dic["이상대응방법"] = "'${dicKeyValue["${sidx}.이상대응방법"]}'";

                    await ca.dicToTable("모니터링설정_스크립트", dic,
                        where: " idx = ${sidx}");
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    String sql = "delete from 모니터링설정_스크립트 where idx = ${sidx}";
                    ca.sqlExecPost(sql);
          ...
          Expanded(
            child: Row(
              children: [
                SizedBox(width: 65,),
                Expanded(child:
                uiMemo(context, "스크립트코드", "스크립트 경로 대신, 스크립트 코드를 입력할 수 있습니다.", "${sidx}.스크립트코드", ),),
              ],
            ),
          ),
        ],
  ...
  Future<bool> doReload() async {
    ...
    try {
      ...
      String sql = "select * from 모니터링설정_스크립트 where 정책ID = '${widget.idx}'";
      ...
      if (dsRows.getRowCount() > 0) {
        for (int rr = 0; rr < dsRows.getRowCount(); rr++) {
          ...
          String spath = drPol.value("스크립트경로");
          List<int> decode = base64.decode(spath);
          spath = utf8.decode(decode);

          String scode = drPol.value("스크립트코드");
          List<int> decode2 = base64.decode(scode);
          scode = utf8.decode(decode2);

          getEdit("${idx}.스크립트설명")!.text = drPol.value("스크립트설명");
          getEdit("${idx}.스크립트경로")!.text = spath;
          getEdit("${idx}.스크립트코드")!.text = scode;
          getEdit("${idx}.스크립트감지간격")!.text = drPol.value("스크립트감지간격");
          getEdit("${idx}.스크립트감지정상")!.text = drPol.value("스크립트감지정상");
          getEdit("${idx}.스크립트감지오류")!.text = drPol.value("스크립트감지오류");
          getEdit("${idx}.재시도횟수")!.text = drPol.value("재시도횟수");
        }
      }
