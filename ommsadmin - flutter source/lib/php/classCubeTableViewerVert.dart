...
class CubeTableViewerVertStatefulWidget extends StatefulWidget {
  String table = "";
  String sql = "";
  ...
  CubeTableViewerVertStatefulWidget(
      {Key? key,
      required this.table,
      required this.sql,
  ...
  @override
  State<CubeTableViewerVertStatefulWidget> createState() =>
      _CubeTableViewerStatefulWidget();
}

class _CubeTableViewerStatefulWidget
    extends State<CubeTableViewerVertStatefulWidget> {
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: ListView.builder(
                ...
                itemCount: ds.colsInfo.length,
                itemBuilder: (BuildContext context, int index) {
                  var cols = getColumns(ds);
                  var rows = getDataCellsInRow(ds, 0);
                  return
                   Row(
                            children: [
                              cols[index],
  ...
  List<Widget> getDataCellsInRow(TDataSet ds, int r) {
    ...
    ds.colsInfo.values.forEach((drCol) {
      ...
      if (ds.colVisible(col)) {
        double colWidth = double.tryParse(ds.colsInfo[col]!.value('Width')) ?? 100;
